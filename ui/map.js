...
function initMap() {
  map = new google.maps.Map(document.getElementById("map"), {
    zoom: 12,
    center: { lat: 40.7128, lng: -74.006 }, // Centro en Nueva York
  });
  ...
  fetch(`/api/tripPredict?id=${id}`)
    ...
    .then((data) => {
      ...
      document.getElementById(
        "trip_fare"
      ).innerHTML = `<b>    Predicted fare $</b> ${data.fare}`;
      document.getElementById(
        "trip_duration"
  ...
  return new Promise((resolve, reject) => {
    ...
    geocoder.geocode({ address: origin }, (resultsOrigin, statusOrigin) => {
      if (statusOrigin === "OK") {
        ...
        geocoder.geocode(
          { address: destination },
          (resultsDestination, statusDestination) => {
                ...
                directionsService.route(
                  ...
                  (response, status) => {
                    if (status === "OK") {
                      ...
                      document.getElementById(
                        "distance"
                      ).innerHTML = `<b>    Distance </b> ${distance.toFixed(
                        2
                      )} km`;
...
async function getWeatherData() {
  try {
    ...
    document.getElementById("weather-icon").src = `${
      weatherConditions[data.current_weather.weathercode][
        day_nigth[data.current_weather.is_day]
      ].image
    }`;
    document.getElementById(
      "temperature"
    ...
    document.getElementById(
      "wind-speed"
    ).textContent = `${data.current_weather.windspeed} km/h`;
    document.getElementById(
      "humidity"
    ).textContent = `${data.hourly.relative_humidity_2m[indice]}%`;
